  ...
  const handleFileInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && (file.type === 'image/jpg' || file.type === 'image/png')) {
      ...
      reader.onload = (e) => {
        ...
        if (typeof result === 'string') {
          setImage(result);
>>>>>>> f837732e5aae99d70ffdfc1f6ef09de7708526f8
        }
      };
      reader.readAsDataURL(file);
    } else {
        ...
        <div>
            ...
            <form action="" onSubmit={handleSubmit} className="flex flex-col w-[60%] mt-[40px] mr-auto ml-auto mb-0">
                ...
                <div onClick={handleClickRef} className="flex flex-col justify-center items-center mb-[15px] mt-[10px] mr-0 ml-0 p-[20px] h-[140px] border-2 border-dashed border-[#4b486a] rounded-[6px] relative bg-[#4b486a4d] cursor-pointer">
                    ...
                    {
                     image ? (
                        <img src={image} alt="Preview" className="w-[50px] h-[50px] border-1 border-solid border-[#8784a4] rounded-[10px] absolute top-[15%]"/>
                     ) : (
                        <img src="/icon-upload.svg" alt="" className="w-[50px] h-[50px] border-1 border-solid border-[#8784a4] rounded-[10px] absolute top-[15%] bg-[#4b486a]"/>
                     )
                    }
